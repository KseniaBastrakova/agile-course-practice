...
public class ViewModelTests {
    ...
    public void operationAddCostsCorrectResult() {
        viewModel.inputExpencesProperty().set("10.37");
        viewModel.operationProperty().set(Expenses.Operation.SETPRODUCTS);
        viewModel.set();
        Calendar date = GregorianCalendar.from(viewModel.dateInputProperty().get().atStartOfDay(ZoneId.systemDefault()));
    ...
    public void operationGetCostsCorrectResult() {
        ...
        DayExpenses actual = new DayExpenses(eatingOutMoney,
                ...
                transportMoney, servicesMoney, entertainmentMoney);
        viewModel.dateInputProperty().get().of(2009, Month.APRIL,10);
        Calendar date = GregorianCalendar.from(viewModel.dateInputProperty().get().atStartOfDay(ZoneId.systemDefault()));
        viewModel.expensesProperty().addCost(actual, date);
    ...
    @Test
    public void statusGettingIsReadyWhenFieldsAreFill1() {
        LocalDate date = LocalDate.of(2009, Month.APRIL,10);
        viewModel.dateInputProperty().set(date);
        viewModel.inputKopekProperty().set("22");
        viewModel.inputRublesProperty().set("12");
